...
This project consists of a Python script designed to update and sort CRM contact data based on event feedback and specific sorting criteria provided by BLACKBIRD. This submitted code and files are generated by JIHYE OH.

# Summary 
 - Generate the unsorted `crm.csv` 
 - Create the sorted `crm_update.csv`. 
 
## Github **additional** https://github.com/JihyeAnnaOh/blackbird.git 

There are 'submission_add/script_add.py' which is add new column for sorting purpose. The new 'feedback action status' column is determined by analyzing the feedback comments for specific keywords. If the feedback contains keywords like "urgent", "immediate", "critical", or "asap", it is categorized as "Urgent Review". If the feedback contains keywords like "improve", "better", "enhance", or "update", it is categorized as "Review for Improvement". If the feedback does not contain any of these keywords, it is categorized as "General Note". This categorization helps prioritize feedback based on its urgency and importance.
...
The script generates the following output files in the `submission` folder:

- **`raw_crm.csv`**: Contains the original CRM data fetched from the API.
- **`raw_form.csv`**: Contains the original form submission data.
- **`crm_update.csv`**: Contains the updated CRM data after merging with form submissions.
- **`crm_update_sorted_date_name.csv`**: Contains the CRM data sorted by date (newest first) and last name.
...
The feedback action status is determined by analyzing the feedback comments for specific keywords. If the feedback contains keywords like "urgent", "immediate", "critical", or "asap", it is categorized as "Urgent Review". If the feedback contains keywords like "improve", "better", "enhance", or "update", it is categorized as "Review for Improvement". If the feedback does not contain any of these keywords, it is categorized as "General Note". This categorization helps prioritize feedback based on its urgency and importance. 